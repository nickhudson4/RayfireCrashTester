        ...
        void Clusterize()
        {
            for (int i = 0; i < cluster.childClusters.Count; i++)
            {
                ...
                else if (meshRootHost != null)
                {
                    ...
                    // Destroy components. Set activated properties.
                    for (int s = 0; s < cluster.childClusters[i].shards.Count; s++)
                    {
                        ...
                        // Set existing Rigid to dynamic in case of cluster demolition
                        cluster.childClusters[i].shards[s].rigid.simulationType = SimType.Dynamic;
                    }
