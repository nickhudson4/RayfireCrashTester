...
namespace RayFire
{
    ...
    public static class RFFragment
    {
        ...
        public static RFShatter CacheMeshes(ref Mesh[] meshes, ref Vector3[] pivots, ref RFDictionary[] rfOrigSubMeshIds, RayfireShatter scrShatter)
        {
            ...
            {
                RayfireMan.Log ("Point Cloud is empty");
                meshes = null;
                pivots = null;
                return null;
            }
            ...
            {
                RayfireMan.Log ("Bad Shatter output mesh: " + scrShatter.name);
                return null;
            }
        ...
        public static void SliceMeshes(ref Mesh[] meshes, ref Vector3[] pivots, ref RFDictionary[] rfOrigSubMeshIds, RayfireRigid scr, List<Vector3> sliceData)
        {
            ...
            if (successState == false)
            {
                scr.meshes = null;
                scr.mshDemol.badMesh++;
                RayfireMan.Log ("Bad mesh: " + scr.name, scr.gameObject);
                return;
            }
            ...
            Mesh mesh, int innerSubId, ref List<Dictionary<int, int>> origSubMeshIDs, Object obj, List<int> markedElements = null)
        {
            ...
            if (meshes == null)
            {
                RayfireMan.Log ("RFShatter: Null mesh", obj);
                meshes         = null;
                pivots         = null;
                origSubMeshIDs = null;
                return false;
            ...
            if (EmptyMeshState (meshes) == true)
            {
                ...
                RayfireMan.Log ("RFShatter: Empty Mesh", obj);
            }

            // Single mesh after mesh fix check
            if (meshes.Length <= 1)
            {
                RayfireMan.Log ("RFShatter: Mesh amount: " + meshes.Length, obj);
                meshes         = null;
                pivots         = null;
                origSubMeshIDs = null;
                return false;
        ...
        static void SetRigidFragmentProperties(RFShatter shatter, RayfireShatter scrSh, RayfireRigid scrRigid)
        {
            ...
            {
                // Get final amount
                int percVar = Random.Range (0, scrRigid.mshDemol.am * scrRigid.mshDemol.var / 100);
                scrRigid.mshDemol.totalAmount = scrRigid.mshDemol.am + percVar;

                // Set Voronoi Uniform properties
                SetVoronoi (shatter, scrRigid.mshDemol.totalAmount, scrRigid.transform, scrRigid.limitations.contactVector3, scrRigid.mshDemol.bias);
                return;
            }
            ...
            // Set properties
            if (scrSh.type == FragType.Voronoi)
                SetVoronoi (shatter, scrRigid.mshDemol.totalAmount, scrSh.transform, scrRigid.limitations.contactVector3, scrSh.voronoi.centerBias);
            else if (scrSh.type == FragType.Splinters)
                SetSplinters (shatter, scrRigid.mshDemol.totalAmount, scrSh.splinters, scrSh.transform, scrRigid.limitations.contactVector3, scrSh.splinters.centerBias);
            else if (scrSh.type == FragType.Slabs)
                SetSlabs (shatter, scrRigid.mshDemol.totalAmount, scrSh.slabs, scrSh.transform, scrRigid.limitations.contactVector3, scrSh.splinters.centerBias);
            else if (scrSh.type == FragType.Radial)
        ...
        static void SetSlices(RFShatter shatter, Transform tm, RFSlice slices)
        {
            ...
            else
            {
                ...
                norms  = list.Select (t => slices.Axis (t)).ToArray();
            }
        ...
        public static void InteractiveStart(RayfireShatter scr)
        {
            ...
            // Cache
            scr.shatterInt = CacheMeshes (ref scr.meshes, ref scr.pivots, ref scr.rfOrigSubMeshIds, scr);
